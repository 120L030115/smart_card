<template>
    <div>
        <Card :bordered="false" dis-hover class="ivu-mt">
            <Input  style="width:300px;float:left;margin-right:20px" v-model="name" placeholder="请输入姓名关键字"  search enter-button @on-search="getData()"/>
        ...
        <Modal v-model="showModel" :title="title" :loading="creating" @on-ok="actionAccount" :transfer="false">
            <Form ref="infoData" :model="infoData" :rules="addRules" :label-width="80">
                <FormItem label="所属用户：" prop="account_id">
                   <Select v-model="infoData.account_id" style="width:200px" placeholder="请选择所属用户">
                        <Option v-for="(item,index) in accountData" :key="index" :value="item.id">{{ item.name }}</Option>
                   </Select> 
                </FormItem>
                <FormItem label="姓名：" prop="name">
                    <Input v-model="infoData.name" placeholder="请输入姓名" />
                </FormItem>
                <FormItem label="职位：" prop="position">
                    <Input v-model="infoData.position" placeholder="请输入职位" />
                </FormItem>
...
<script>
    ...
    import { personal_infoList, personal_infoAdd, personal_infoDelete, personal_infoUpdate } from '@api/personal_info';
    ...
    export default {
        ...
        methods: {
            ...
            async getData () {
                ...
                let res = await personal_infoList(params);
                if (res.errno === 0) {
            ...
            async actionAccount () {
                this.$refs.infoData.validate(async (valid) => {
                    if (valid) {
                        ...
                        let res = await personal_infoAdd(this.infoData);
                        if (res.errno === 0) {
            ...
            remove (id, name) {
                this.$Modal.confirm({
                    title: '删除名片',
                    content: '确定删除名片' + name + '吗？',
                    onOk: async () => {
                        let res = await personal_infoDelete({ id: id });
                        if (res.errno === 0) {
                            this.$Message.success(res.data);
                            this.getData()
                        } else {
            ...
            async cP () {
                    ...
                    let res = await personal_infoUpdate(params);
                    if (res.errno === 0) {
                        this.$Message.success(res.data);
                        this.changeac = false;
                        this.getData()
